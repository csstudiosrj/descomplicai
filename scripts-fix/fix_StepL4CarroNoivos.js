/**
 * Script de correção segura para StepL4CarroNoivos.jsx
 * Substitui hardcode "noivos" por termos dinâmicos
 * 
 * USO: node fix_StepL4CarroNoivos.js <caminho-do-arquivo>
 */
...
if (!filePath) {
  console.error('Uso: node fix_StepL4CarroNoivos.js <caminho-do-arquivo>');
  process.exit(1);
}
...
content = content.replace(
  /aria-label="Carro dos noivos"/g,
  'aria-label={`Carro dos ${termos.pessoa1} e ${termos.pessoa2}`}'
);

// 2. Título
content = content.replace(
  /"Qual o carro dos noivos\?"/g,
  '{`Qual o carro dos ${termos.pessoa1} e ${termos.pessoa2}?`}'
);
content = content.replace(
  /"Carro dos noivos"/g,
