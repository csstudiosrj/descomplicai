...
const ARVORES_MAP = {
  ...
  // CORREÇÃO: noAnterior agora retorna o nó anterior E o histórico atualizado
  // (sem o nó atual, para que a próxima chamada volte mais um)
  export function noAnterior(historico, arvore) {
    if (!historico || historico.length < 2) return null;

    // Remove o último item (nó atual) do histórico
    const novoHistorico = historico.slice(0, -1);
    // O novo "atual" é o último do histórico reduzido
    const idAnterior = novoHistorico[novoHistorico.length - 1];
    const no = getNoPorId(idAnterior, arvore);
