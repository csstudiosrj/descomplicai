 ...
 * - Fornecedor excluído → marca fornecedor_excluido = true
 *
...
export async function sincronizarFornecedorComFinanceiro(fornecedor, supabaseAdmin) {
    if (!fornecedor || !supabaseAdmin) {
      throw new Error('fornecedor e supabaseAdmin são obrigatórios');
    }
  
    const { id, evento_id, usuario_id, nome, categoria_principal, valor_total, status } = fornecedor;
  
    // Busca se já existe despesa sincronizada para este fornecedor
    ...
    // Se fornecedor foi excluído (status = null ou flag de exclusão)
    ...
    const payload = {
      ...
      categoria: categoria_principal || 'outro',
  ...
  /**
   * Marca despesa como excluída quando fornecedor é deletado.
