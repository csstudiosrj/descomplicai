...
export default function FornecedorLoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: fornData, error: fornErr } = await supabaseClient
      ...
      setErro('Esta conta não é de fornecedor.');
    } catch (err) {
      setEnviando(false);
      console.error('[forn/login] erro:', err);
  ...
  const handleRecuperarSenha = async () => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: typeof window !== 'undefined' ? window.location.origin + '/fornecedor/login' : undefined,
    });
    if (error) {
  ...
  return (
    <>
      <Head>
        <title>Entrar — Fornecedor | Descomplicaí</title>
        <meta name="description" content="Acesse seu painel de fornecedor no Descomplicaí." />
      </Head>
        ...
        <div style={{ width: '100%', maxWidth: '400px' }}>
          <div style={{ textAlign: 'center', marginBottom: 'var(--space-8)' }}>
            ...
            <p style={{
              ...
              Conecte-se com noivas que planejam o casamento
            </p>
          </div>
