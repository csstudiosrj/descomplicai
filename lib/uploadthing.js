/**
 * Configuracao do UploadThing — versao v7+ (API moderna)
 * @module lib/uploadthing
 * 
 * Usa a API v7 do UploadThing com presigned URLs.
 * Funcoes com sufixo "ViaAPI" sao para uso no browser (chamam /api/uploadthing).
 * Funcoes sem sufixo sao para uso no server-side (usam UPLOADTHING_TOKEN direto).
 */
...
/**
 * Gera URL pre-assinada para upload direto (API v7 UploadThing)
 ...
 */
export async function gerarPresignedURL(fileName, fileType, fileSize) {
  if (!UPLOADTHING_TOKEN) {
    console.warn('UPLOADTHING_TOKEN nao configurado');
    throw new Error('UploadThing nao configurado');
  }

  const res = await fetch('https://api.uploadthing.com/v7/prepareUpload', {
    method: 'POST',
    headers: {
 ...
 * @returns {Promise<{url: string, fields: Object}>}
 */
export async function gerarPresignedURLViaAPI(fileName, fileType, fileSize) {
  const basePath = process.env.NEXT_PUBLIC_BASE_PATH || '';
  const res = await fetch(basePath + '/api/uploadthing', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ filename: fileName, filetype: fileType, filesize: fileSize }),
  });
...
export async function uploadArquivoViaAPI(file, opcoes = {}) {
  ...
  const { url, fields } = await gerarPresignedURLViaAPI(nomeUnico, arquivoProcessado.type, arquivoProcessado.size);
 ...
 * SERVER-SIDE ONLY — usa process.env.UPLOADTHING_TOKEN direto
 * @param {File} file
 * @param {Object} opcoes { tipo: 'perfil'|'referencia'|'pdf' }
 * @returns {Promise<{url: string, nome: string}>}
 */
export async function uploadArquivo(file, opcoes = {}) {
  ...
  const { url, fields } = await gerarPresignedURL(nomeUnico, arquivoProcessado.type, arquivoProcessado.size);
  const resultado = await fazerUpload(arquivoProcessado, url, fields);
...
export async function uploadBuffer(buffer, fileName, fileType) {
  if (!UPLOADTHING_TOKEN) {
    throw new Error('UPLOADTHING_TOKEN nao configurado');
  }

  const { url, fields } = await gerarPresignedURL(fileName, fileType, buffer.length);
