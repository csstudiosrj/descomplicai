...
ORDEM DE TRABALHO — O QUE FAZER AGORA
ETAPA 1 — Pagamento (rápido, desbloqueia os testes)
Remover integração MP SDK do pages/memorial/conclusao.jsx
Substituir por link de pagamento (PIX + cartão) hardcoded temporariamente
Testar fluxo completo questionário → conclusão → link de pagamento
ETAPA 2 — Cadastro/Perfil antes do questionário
Criar pages/memorial/perfil.jsx — coleta nome, data, cidade, orçamento
Criar evento no banco AQUI após login
Redirecionar para /memorial após criar evento
Remover criação de evento de dentro do questionário
ETAPA 3 — Novo motor do questionário
Definir config de cada step (fase, tipo, dependeDe, campo)
Reescrever utils/algoritmo.js para ler configs automaticamente
Reescrever components/memorial/MemorialOrchestrator.jsx para Fase 1 + Fase 2
NÃO mexer nos componentes visuais dos steps — só adicionar export config
ETAPA 4 — Imagens dos cards
Fase 1 (swipe): 1 imagem grande por pergunta, composição segura no centro
Fase 2 (aprofundamento): imagens abstratas nos cards menores
Geração: SVG abstratos ou ChatGPT em lotes
ETAPA 5 — Validação completa
Testar questionário completo
Testar painel cerimonialista
Testar painel fornecedor
...
MP_ACCESS_TOKEN (manter mas não usar no SDK)
NEXT_PUBLIC_SITE_URL=https://arxum.csstudios.site/descomplicai
NEXT_PUBLIC_PUBLIC_BASE_PATH=/descomplicai
PROBLEMAS PENDENTES — LOGO DO HEADER
Data: 12/07/2026
Status: NÃO RESOLVIDO — deixado quieto por enquanto
Arquivo: components/ui/Header.jsx
Comportamento atual (errado)
Table
Situação	Comportamento esperado	Comportamento real
Logado no memorial	Voltar pro início do memorial (Step00) ou home	Tenta ir pro /painel (errado — usuário pode não ser assinante)
Deslogado no memorial	Ir pra home /	Vai pra arxum.csstudios.site (URL errada, possivelmente problema de basePath)
Fora do memorial	Ir pra home /	Funciona corretamente
Tentativas feitas
Tentativa 1: Evento interno memorial-reset que o MemorialOrchestrator escutava e chamava irParaEtapa(0). Rejeitado — resetaria o progresso do usuário logado.
Tentativa 2: Link do Next.js com href condicional (/ ou /painel). Falhou — o Next.js faz shallow navigation na mesma rota, não sai do /memorial.
Tentativa 3: router.push('/painel') para logado e window.location.href = '/' para deslogado, usando <button> em vez de Link. Falhou — logado vai pro painel (errado para não-assinantes), deslogado vai pra URL errada.
Causa raiz suspeita
O basePath do Next.js (/descomplicai) pode estar interferindo no window.location.href = '/'
O Link do Next.js intercepta navegação interna e faz shallow reload quando href é / estando em /memorial
Não há decisão de produto sobre para onde o logo deve direcionar quando o usuário está no memorial (home? Step00? Painel?)
Próximos passos (quando retomar)
Decidir comportamento correto com base no estado do usuário:
Logado + assinante → /painel
Logado + não-assinante (trial/memorial) → /memorial (Step00) ou home /
Deslogado → home /
Usar window.location.assign() com URL completa incluindo basePath, ou router.push() com path absoluto
