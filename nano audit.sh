...
# 1. GIT — últimos commits e branch atual
echo ">>> 1. GIT STATUS"
echo "Branch atual:"
...
# 2. FILE TREE — estrutura de arquivos relevantes
...
# 3. CONTEÚDO DOS ARQUIVOS CHAVE (head + tail para identificar existência)
...
# 4. VERIFICAR SE HÁ ARQUIVOS DE PENDÊNCIAS
...
# 5. VERIFICAR CSS VARIABLES (zero hex hardcoded)
echo ">>> 5. CSS VARIABLES — Verificar se existem"
...
# 6. VERIFICAR SE HÁ TABELAS DE MESAS (outro chat)
...
if [ -f ".env.local" ]; then
  ...
  echo ".env.local: NÃO existe (OK — regra do projeto)"
fi
echo ""

# 8. PACKAGE.JSON — verificar dependências
