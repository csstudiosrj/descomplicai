...
const OPCOES = [
  ...
  { valor: 'separados', label: 'Separados', subtexto: 'Um entra primeiro, depois o outro', icone: 'user' },
  { valor: 'surpresa', label: 'Surpresa', subtexto: 'Entrada inesperada ou diferente', icone: 'zap' },
];
...
export default function Step30Entrada({ onSelect, estadoAtual }) {
  ...
  return (
    <div role="radiogroup" aria-label={`Como será a entrada de ${termos.pessoa1} e ${termos.pessoa2}?`} style={{ maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', animation: 'fadeInUp 300ms ease-out' }}>
      ...
      <div>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-3xl)', lineHeight: 'var(--leading-tight)', color: 'var(--color-text-primary)', marginBottom: 'var(--space-2)' }}>{`Entrada de ${termos.pessoa1} e ${termos.pessoa2}`}</h1>
        <p style={{ fontFamily: 'var(--font-body)', color: 'var(--color-text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>Como será a entrada?</p>
      </div>
...
Step30Entrada.propTypes = { onSelect: PropTypes.func.isRequired, estadoAtual: PropTypes.object };
