...
const OPCOES = [
    {
      valor: 'sim',
      label: 'Sim, completa',
      subtexto: 'Monograma, fontes, cores, papelaria',
      icone: 'pen',
    },
    {
      valor: 'basica',
      label: 'Básica',
      subtexto: 'Apenas convite com design simples',
      icone: 'type',
    },
    {
      valor: 'nao',
      label: 'Não',
      subtexto: 'Sem identidade visual definida',
      icone: 'x',
    },
];

export default function Step50IdentidadeVisual({ onSelect, estadoAtual }) {
  const selecionado = estadoAtual?.monograma;

  return (
    <div
      role="radiogroup"
      aria-label="Desejam identidade visual personalizada?"
      style={{
        maxWidth: '640px',
        margin: '0 auto',
        display: 'flex',
    ...
    >
      ...
      <div>
        ...
        <p style={{ fontFamily: 'var(--font-body)', color: 'var(--color-text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>
          Desejam identidade visual personalizada?
        </p>
      </div>
        ...
        {OPCOES.map((opcao) => {
          ...
          return (
            <Card
              ...
              selected={isSelected}
              padding="lg"
              onClick={() => onSelect('monograma', opcao.valor)}
              role="radio"
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  onSelect('monograma', opcao.valor);
                }
              }}
            >
...
Step50IdentidadeVisual.propTypes = {
  onSelect: PropTypes.func.isRequired,
  estadoAtual: PropTypes.object,
};

export { Step50IdentidadeVisual };
