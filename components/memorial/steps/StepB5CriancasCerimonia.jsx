...
const OPCOES = [
    {
      valor: 'True',
      label: 'Sim',
      subtexto: 'Daminhas, pajens, porta-alianças',
      icone: 'baby',
    },
    {
      valor: 'False',
      label: 'Não',
      subtexto: 'Cerimônia apenas com adultos',
      icone: 'x',
    },
];

export default function StepB5CriancasCerimonia({ onSelect, estadoAtual }) {
  const selecionado = estadoAtual?.criancasCerimonia;

  return (
    <div
      role="radiogroup"
      aria-label="Terão crianças na cerimônia?"
      style={
        maxWidth: '640px',
        margin: '0 auto',
        display: 'flex',
        ...
        {OPCOES.map((opcao) => {
          ...
          return (
            <Card
              ...
              selected={isSelected}
              padding="lg"
              onClick={() => onSelect('criancasCerimonia', opcao.valor)}
              role="radio"
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  onSelect('criancasCerimonia', opcao.valor);
                }
              }}
            >
...
StepB5CriancasCerimonia.propTypes = {
  onSelect: PropTypes.func.isRequired,
  estadoAtual: PropTypes.object,
};

export { StepB5CriancasCerimonia };
