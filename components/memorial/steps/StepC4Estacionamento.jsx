...
const OPCOES = [
    {
      valor: 'sim',
      label: 'Sim',
      subtexto: 'Estacionamento disponível no local',
      icone: 'car',
    },
    {
      valor: 'nao',
      label: 'Não',
      subtexto: 'Sem estacionamento próprio',
      icone: 'x',
    },
    {
      valor: 'valet',
      label: 'Valet',
      subtexto: 'Serviço de manobrista',
      icone: 'user',
    },
];

export default function StepC4Estacionamento({ onSelect, estadoAtual }) {
  const selecionado = estadoAtual?.estacionamento;

  return (
    <div
      role="radiogroup"
      aria-label="Como será o estacionamento?"
      style={
        maxWidth: '640px',
        margin: '0 auto',
        display: 'flex',
        ...
        {OPCOES.map((opcao) => {
          ...
          return (
            <Card
              ...
              selected={isSelected}
              padding="lg"
              onClick={() => onSelect('estacionamento', opcao.valor)}
              role="radio"
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  onSelect('estacionamento', opcao.valor);
                }
              }}
            >
...
StepC4Estacionamento.propTypes = {
  onSelect: PropTypes.func.isRequired,
  estadoAtual: PropTypes.object,
};

export { StepC4Estacionamento };
