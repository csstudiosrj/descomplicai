...
const OPCOES = [
    {
      valor: true,
      label: 'Sim',
      subtexto: 'Final mágico para a festa',
      icone: 'sparkle',
    },
    {
      valor: false,
...
export default function StepH3FogosSparklers({ onSelect, estadoAtual }) {
  const selecionado = estadoAtual?.fogosSparklers;

  return (
    <div
      role="radiogroup"
      aria-label="Desejam fogos ou sparklers?"
      style={{
        maxWidth: '640px',
        margin: '0 auto',
        display: 'flex',
        ...
        {OPCOES.map((opcao) => {
          ...
          return (
            <Card
              ...
              selected={isSelected}
              padding="lg"
              onClick={() => onSelect('fogosSparklers', opcao.valor)}
              role="radio"
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  onSelect('fogosSparklers', opcao.valor);
                }
              }}
            >
...
StepH3FogosSparklers.propTypes = {
  onSelect: PropTypes.func.isRequired,
  estadoAtual: PropTypes.object,
};

export { StepH3FogosSparklers };
