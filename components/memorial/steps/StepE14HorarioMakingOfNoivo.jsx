...
export default function StepE14HorarioMakingOfNoivo({ onSelect, estadoAtual }) {
  const [valor, setValor] = useState(estadoAtual?.horarioMakingOfNoivo || '');

  const handleConfirmar = () => {
    onSelect('horarioMakingOfNoivo', valor.trim());
  };
  ...
  return (
    <div style={{ maxWidth: '480px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 'var(--space-5)', animation: 'fadeInUp 300ms ease-out' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        <Input label="Horário" value={valor} onChange={(e) => setValor(e.target.value)} placeholder="Ex: 12:00" required />
      </div>
...
StepE14HorarioMakingOfNoivo.propTypes = {
  onSelect: PropTypes.func.isRequired,
  estadoAtual: PropTypes.object,
};

export { StepE14HorarioMakingOfNoivo };
