...
const OPCOES = [
    {
      valor: 'sim',
      label: 'Sim',
      subtexto: 'Ônibus/van para todos',
      icone: 'bus',
    },
    {
      valor: 'nao',
      label: 'Não',
      subtexto: 'Convidados vão por conta própria',
      icone: 'x',
    },
    {
      valor: 'alguns',
      label: 'Alguns',
      subtexto: 'Apenas para convidados de fora',
      icone: 'users',
    },
];

export default function StepL5TransporteConvidados({ onSelect, estadoAtual }) {
  const selecionado = estadoAtual?.transporteConvidados;

  return (
    <div
      role="radiogroup"
      aria-label="Terão transporte para convidados?"
      style={{
        maxWidth: '640px',
        margin: '0 auto',
        display: 'flex',
    ...
    >
      ...
      <div>
        ...
        <p style={{ fontFamily: 'var(--font-body)', color: 'var(--color-text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>
          Terão transporte para convidados?
        </p>
      </div>
        ...
        {OPCOES.map((opcao) => {
          ...
          return (
            <Card
              ...
              selected={isSelected}
              padding="lg"
              onClick={() => onSelect('transporteConvidados', opcao.valor)}
              role="radio"
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  onSelect('transporteConvidados', opcao.valor);
                }
              }}
            >
...
StepL5TransporteConvidados.propTypes = {
  onSelect: PropTypes.func.isRequired,
  estadoAtual: PropTypes.object,
};

export { StepL5TransporteConvidados };
