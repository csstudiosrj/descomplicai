...
const OPCOES = [
    ...
    },
    {
      valor: 'nao',
      label: 'Não',
      subtexto: 'Ainda não escolhemos',
      icone: 'x',
    },
    {
      valor: 'buscando',
      label: 'Buscando',
      subtexto: 'Estamos pesquisando opções',
      icone: 'search',
    },
];

export default function StepL1Aliancas({ onSelect, estadoAtual }) {
  const selecionado = estadoAtual?.aliancasEscolhidas;

  return (
    <div
      role="radiogroup"
      aria-label="As alianças estão escolhidas?"
      style={
        maxWidth: '640px',
        margin: '0 auto',
        display: 'flex',
        ...
        {OPCOES.map((opcao) => {
          ...
          return (
            <Card
              ...
              selected={isSelected}
              padding="lg"
              onClick={() => onSelect('aliancasEscolhidas', opcao.valor)}
              role="radio"
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  onSelect('aliancasEscolhidas', opcao.valor);
                }
              }}
            >
...
StepL1Aliancas.propTypes = {
  onSelect: PropTypes.func.isRequired,
  estadoAtual: PropTypes.object,
};

export { StepL1Aliancas };
