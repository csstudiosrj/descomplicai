    ...
    <div role="radiogroup" aria-label="Desejam flores na decoração?" style={{ maxWidth: '640px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', animation: 'fadeInUp 300ms ease-out' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(1, 1fr)', gap: 'var(--space-4)' }}>
        {OPCOES.map((opcao) => {
          ...
          return (
            <Card key={String(opcao.valor)} interactive selected={isSelected} padding="lg" onClick={() => onSelect('flores', opcao.valor)} role="radio" aria-checked={isSelected} tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onSelect('flores', opcao.valor); } }}>
