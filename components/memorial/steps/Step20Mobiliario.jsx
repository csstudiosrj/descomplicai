...
export default function Step20Mobiliario({ onSelect, estadoAtual }) {
  const selecionado = estadoAtual?.mobiliarioEspecial;

  return (
    <div
      role="radiogroup"
      aria-label="Precisam de mobiliário especial?"
      style={{
        maxWidth: '640px',
        margin: '0 auto',
        display: 'flex',
    ...
    >
      ...
      <div>
        ...
        <p style={{ fontFamily: 'var(--font-body)', color: 'var(--color-text-secondary)', lineHeight: 'var(--leading-relaxed)' }}>
          Precisam de mobiliário especial?
        </p>
      </div>
        ...
        {OPCOES.map((opcao) => {
          ...
          return (
            <Card
              ...
              selected={isSelected}
              padding="lg"
              onClick={() => onSelect('mobiliarioEspecial', opcao.valor)}
              role="radio"
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  onSelect('mobiliarioEspecial', opcao.valor);
                }
              }}
            >
...
Step20Mobiliario.propTypes = {
  onSelect: PropTypes.func.isRequired,
  estadoAtual: PropTypes.object,
};

export { Step20Mobiliario };
