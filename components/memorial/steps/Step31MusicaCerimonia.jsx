...
const OPCOES = [
    {
      valor: 'quarteto',
      label: 'Quarteto de cordas',
      subtexto: 'Clássico e sofisticado',
      icone: 'music',
    },
    {
      valor: 'violao',
      label: 'Violão/violino solo',
      subtexto: 'Íntimo e emocionante',
      icone: 'music',
    },
    {
      valor: 'playlist',
      label: 'Playlist gravada',
      subtexto: 'Músicas escolhidas por vocês',
      icone: 'list',
    },
    {
      valor: 'coral',
      label: 'Coral',
      subtexto: 'Grande e impactante',
      icone: 'users',
    },
];

export default function Step31MusicaCerimonia({ onSelect, estadoAtual }) {
  const selecionado = estadoAtual?.musicaCerimonia;

  return (
    <div
      role="radiogroup"
      aria-label="Qual música para a cerimônia?"
      style={{
        maxWidth: '640px',
        margin: '0 auto',
        display: 'flex',
        ...
        {OPCOES.map((opcao) => {
          ...
          return (
            <Card
              ...
              selected={isSelected}
              padding="lg"
              onClick={() => onSelect('musicaCerimonia', opcao.valor)}
              role="radio"
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  onSelect('musicaCerimonia', opcao.valor);
                }
              }}
            >
...
Step31MusicaCerimonia.propTypes = {
  onSelect: PropTypes.func.isRequired,
  estadoAtual: PropTypes.object,
};

export { Step31MusicaCerimonia };
