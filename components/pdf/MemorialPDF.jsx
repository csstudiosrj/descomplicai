...
export function MemorialPDF({ memorial, dadosEvento, usarFontesNativas = false, qrCodeDataUri = null, dimensoesImagens = {} }) {
  ...
  const getSecaoExata = (tituloEsperado) => {
    ...
    return secoes.find(s => {
      ...
      // Match exato ou parcial mais específico
  ...
  const imagemDecoracao = getImagem('decoracao', estilo);
  const imagemCerimonia = getImagem('cerimonia', estilo);
  const imagemFlores = getImagem('flores', flores) || getImagem('flores', 'default');
  const imagemMesa = getImagem('mesaPosta', estilo);
  const imagemAlimentacao = getImagem('alimentacao', estilo);
  const imagemEntretenimento = getImagem('entretenimento', estilo);
  const imagemVestido = getImagem('vestido', dadosEvento?.estiloVestido) || getImagem('vestido', 'default');
  const imagemPapelaria = getImagem('papelaria', estilo);
