...
function calcularImportPath(arquivoPath) {
  ...
  if (profundidade === 0) return './utils/fetchAPI';
  if (profundidade === 1) return '../utils/fetchAPI';
  if (profundidade === 2) return '../../utils/fetchAPI';
  if (profundidade === 3) return '../../../utils/fetchAPI';
  if (profundidade === 4) return '../../../../utils/fetchAPI';
  return '../'.repeat(profundidade) + 'utils/fetchAPI';
}
...
function substituirFetchs(conteudo) {
  ...
  // Mas NAO substitui fetchAPI('/api/... que ja foi substituido
...
function processarArquivo(arquivoPath, backupDir) {
  ...
  const importPath = calcularImportPath(arquivoPath);
...
function main() {
  ...
  console.log('  FIX BASEPATH — Substituicao automatica (Node.js)');
  console.log('='.repeat(50));
  console.log();

  if (!fs.existsSync('next.config.js')) {
    console.log('ERRO: Execute na raiz do projeto (onde esta next.config.js)');
    process.exit(1);
  }
  ...
  const backupDir = path.join('.backup', 'fix-basepath-node');
  fs.mkdirSync(backupDir, { recursive: true });

  console.log();
  console.log('[2/4] Encontrando arquivos com fetch(\'/api/...\')...');
  ...
  if (arquivos.length === 0) {
    console.log();
    console.log('Nenhum arquivo com fetch(\'/api/...\') encontrado. Nada a fazer.');
    process.exit(0);
  }

  console.log();
  console.log('[3/4] Fazendo backup e substituindo...');

  let modificados = 0;
  for (let i = 0; i < arquivos.length; i++) {
    const arquivo = arquivos[i];
    console.log(`      [${i + 1}/${arquivos.length}] ${path.relative(process.cwd(), arquivo)}`);
  ...
  console.log();
  console.log('[4/4] Verificando se sobrou algum fetch(\'/api/...\')...');
  ...
  if (restantes.length > 0) {
    console.log();
    console.log('ALERTA: Ainda existem fetch(\'/api/...\') nao substituidos:');
    for (const r of restantes) console.log(`  - ${r}`);
  } else {
    console.log('      Nenhum fetch(\'/api/...\') restante encontrado.');
  }
  ...
  console.log(`Arquivos modificados: ${modificados}`);
  console.log(`Backups salvos em: ${backupDir}`);
  console.log();
  console.log('Proximos passos:');
  console.log('  1. npm run build');
  console.log('  2. Se passar: git add . && git commit -m "fix: basePath em todas as chamadas de API"');
